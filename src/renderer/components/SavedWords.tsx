...
export function SavedWords({ words, onSelect, onRemove }: SavedWordsProps) {
  if (words.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in duration-700">
        <p className="font-sans text-[12px] font-medium tracking-[1px] uppercase text-text-faint mb-3">Nenhuma palavra salva</p>
        <p className="font-serif text-sm text-text-muted italic max-w-[250px]">
          As palavras que você marcar como favoritas aparecerão aqui.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <p className="section-label px-1">Seu Vocabulário ({words.length})</p>
        ...
        {words.map((w) => (
          ...
          >
            <div className="flex flex-col gap-1">
              ...
              <p className="font-sans text-[12px] text-text-muted truncate max-w-[300px]">
                {w.meaning_pt}
              </p>
            </div>
            ...
            <button 
              ...
              className="w-8 h-8 flex items-center justify-center text-text-faint hover:text-[#D85A30] transition-colors"
              title="Remover"
            >
